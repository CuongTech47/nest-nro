...
import com.ngocrong.ngocrong.command.CommandManager;
...
public class MessageHandler implements IMessaggeHandler{
            ...
            try {
                switch (command) {
                    case Cmd.SUB_COMMAND:
                        ...
                        break;
                    case Cmd.LOGIN2:
                        service.dialogMessage("Đăng ký tài khoản tại Ngọc Rồng Namek");
                        break;
                    case Cmd.NOT_LOGIN:
                        messageNotLogin(mss);
                        break;
    ...
    private void messageNotMap(Message mss) throws IOException {
        if (mss != null) {
    ...
    private void messageSubCommand(Message mss) throws IOException {
        if (mss != null) {
            byte command = mss.reader().readByte();
            try {
                if (_char == null) {
